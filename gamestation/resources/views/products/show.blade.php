    ...
    <section class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="{{ route('products.index') }}" class="ml-1 text-sm font-medium text-slate-600 hover:text-slate-900 md:ml-2">Sản phẩm</a>
                    </div>
                </li>
        ...
        <div class="grid gap-8 lg:grid-cols-3 mb-8">
                    ...
                    <div class="relative bg-slate-50 aspect-square">
                        ...
                        @if($product->stock <= 0)
                            <div class="absolute inset-0 bg-black/50 flex items-center justify-center">
                                <span class="text-2xl font-bold text-white">Hết hàng</span>
                            </div>
                        @endif
                    </div>
            ...
            <div>
                ...
                <div class="mb-8 rounded-lg bg-gradient-to-r from-sky-50 to-blue-50 p-6 border border-sky-200">
                    <div class="mb-4">
                        <span class="text-sm font-semibold text-slate-600 uppercase tracking-wider">Giá bán</span>
                    ...
                    <div class="flex items-center justify-between pt-4 border-t border-sky-200">
                        <span class="text-sm text-slate-600">Tồn kho:</span>
                ...
                @auth
                    @if($product->stock > 0)
                        <form method="POST" action="{{ route('cart.add') }}" class="space-y-3 mb-6">
                            ...
                            <!-- Buy Now Button -->
                            <button type="submit" name="buy_now" value="1" class="block w-full text-center rounded-lg border-2 border-slate-300 px-5 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50 transition-colors">
                            ...
                            <a href="{{ route('contact', ['product_id' => $product->id]) }}" class="block w-full text-center mt-2 rounded-lg border-2 border-slate-200 px-5 py-3 text-sm font-semibold text-slate-600 hover:bg-slate-50 transition-colors">
                                <i class="fas fa-comment-dots mr-2"></i>Tư vấn nhanh
                            </a>
                        </form>
                    @else
                        <div class="mb-6 p-4 bg-red-50 border-l-4 border-red-500 rounded">
                            <p class="text-sm font-semibold text-red-700">Sản phẩm hiện đã hết hàng</p>
                        </div>
                    @endif
                    @else
                    <div class="mb-6 space-y-3">
                        <div class="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                            <p class="text-sm text-blue-900 mb-3">Vui lòng đăng nhập để mua sản phẩm</p>
                            <a href="{{ route('login', ['next_action' => 'cart_add', 'product_id' => $product->id, 'quantity' => 1]) }}" class="w-full block text-center gs-button py-3 font-semibold rounded-lg transition-all hover:shadow-lg">
                                <i class="fas fa-sign-in-alt mr-2"></i>Đăng nhập để thêm giỏ hàng
                            </a>
                            <a href="{{ route('login', ['next_action' => 'buy_now', 'product_id' => $product->id, 'quantity' => 1]) }}" class="w-full block text-center mt-2 rounded-lg border-2 border-blue-300 px-5 py-3 text-sm font-semibold text-blue-700 hover:bg-blue-50 transition-colors">
                                <i class="fas fa-bolt mr-2"></i>Đăng nhập để mua ngay
                            </a>
        ...
        <div class="mt-8 mb-12">
            ...
            <div id="description-tab" class="tab-content">
                <div class="gs-card bg-white p-6 md:p-8">
                    ...
                    @else
                        <p class="text-center text-slate-500 py-8">Chưa có thông số chi tiết cho sản phẩm này.</p>
                    @endif
                </div>
            </div>
            ...
            <div id="reviews-tab" class="tab-content hidden">
                <div class="gs-card p-8 bg-white">
                    ...
                    <div class="mb-8 space-y-6">
                        @forelse($product->reviews as $review)
                            <div class="border-b border-slate-200 pb-6 last:border-b-0">
                                <div class="mb-3 flex items-start justify-between">
                                    <div>
                                        <p class="font-semibold text-slate-900">{{ $review->user->name ?? 'Người dùng' }}</p>
                                ...
                                @if($review->admin_reply)
                                    <div class="mt-4 rounded-lg bg-blue-50 border-l-4 border-blue-500 p-4">
                                        <p class="text-xs font-semibold text-blue-900 uppercase tracking-wider mb-2">💬 Phản hồi từ Admin</p>
                        ...
                        @empty
                            <p class="py-8 text-center text-slate-500">Chưa có đánh giá nào. Hãy là người đầu tiên!</p>
                        @endforelse
                    </div>
                    ...
                    @auth
                        <div class="border-t border-slate-200 pt-8">
                            <h3 class="mb-6 text-lg font-semibold text-slate-900">Viết đánh giá của bạn</h3>
                            <form method="POST" action="{{ route('reviews.store', $product) }}" class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <select name="rating" class="w-full rounded-lg border-2 border-slate-300 px-4 py-2 text-base focus:border-sky-500 focus:outline-none">
                                        ...
                                        <option value="3" selected>⭐⭐⭐ 3 sao - Tốt</option>
                                ...
                                <div>
                                    <label class="block text-sm font-semibold text-slate-700 mb-3">Bình luận của bạn</label>
                                    <textarea name="comment" rows="5" 
                                              ...
                                              placeholder="Chia sẻ trải nghiệm thực tế của bạn với sản phẩm này...">{{ old('comment') }}</textarea>
                                    @error('comment')
                    ...
                    @else
                        <div class="border-t border-slate-200 pt-8">
                            <p class="text-center text-slate-600">
                                <a href="{{ route('login') }}" class="font-semibold text-sky-600 hover:text-sky-700">Đăng nhập</a> 
                                để viết đánh giá
                            </p>
                        </div>
                    @endauth
        ...
        @if($related->count() > 0)
            <div class="mt-16 mb-14">
                <h2 class="text-center font-extrabold text-slate-900" style="font-size: 28px; line-height: 1.25; margin-bottom: 48px;">Sản phẩm liên quan</h2>
